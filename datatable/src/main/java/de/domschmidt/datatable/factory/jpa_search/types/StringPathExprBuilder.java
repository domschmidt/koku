...
public class StringPathExprBuilder implements IExpressionSearch<StringExpression> {
    ...
    public StringExpression getCastedExpression(
        ...
        if (StringExpression.class.isAssignableFrom(expression.getClass())) {
            return (StringExpression) expression;
        } else {
            return Expressions.asString((Expression<String>)expression);
        }
    }
    ...
    public BooleanExpression createExpression(
        ...
        switch (op) {
            case LT:
                result = getCastedExpression(expression).trim().lt(castedValue);
                break;
            case LOE:
                result = getCastedExpression(expression).trim().loe(castedValue);
                break;
            case GT:
