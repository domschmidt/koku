...
public class IntegerPathExprBuilder implements IExpressionSearch<NumberExpression<Integer>> {

    @Override
    public NumberExpression<Integer> getCastedExpression(
            final Expression<?> expression
    ) throws DataTableUnsupportedExpressionTypeException {
        if (!NumberExpression.class.isAssignableFrom(expression.getClass()) || !expression.getType().equals(Integer.class)) {
            throw new DataTableUnsupportedExpressionTypeException("NumberExpression is not assignable to " + expression.getClass());
        }
        return (NumberExpression<Integer>) expression;
    }
    ...
    public BooleanExpression createExpression(
        ...
        final int castedValue;
        try {
            castedValue = Integer.parseInt(rawSearchValue.toString());
        } catch (final NumberFormatException e) {
            throw new DataTableUnsupportedExpressionTypeException("Cannot extract Integer of given search value \"" + rawSearchValue + "\"", e);
        }
