...
public class DoublePathExprBuilder implements IExpressionSearch<NumberExpression<Double>> {

    @Override
    public NumberExpression<Double> getCastedExpression(
            final Expression<?> expression
    ) throws DataTableUnsupportedExpressionTypeException {
        if (!NumberExpression.class.isAssignableFrom(expression.getClass()) || !expression.getType().equals(Double.class)) {
            throw new DataTableUnsupportedExpressionTypeException("NumberExpression is not assignable to " + expression.getClass());
        }
        return (NumberExpression<Double>) expression;
    }
    ...
    public BooleanExpression createExpression(
        ...
        final Double castedValue;
        try {
            castedValue = Double.valueOf(rawSearchValue.toString());
        } catch (final NumberFormatException e) {
            throw new DataTableUnsupportedExpressionTypeException("Cannot extract Double of given search value \"" + rawSearchValue + "\"");
        }
