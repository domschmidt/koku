...
public class CustomerAppointmentKafkaMaintenanceStartupListenerService
        implements ApplicationListener<KafkaStreamsRunningEvent> {

    final CustomerAppointmentKafkaMaintenanceService executor;

    @Override
    public void onApplicationEvent(KafkaStreamsRunningEvent event) {
        CompletableFuture.delayedExecutor(5, TimeUnit.MINUTES).execute(executor::runMaintenance);
    }
}
