...
public class CustomerAppointmentToKafkaCustomerAppointmentDtoTransformer {
    ...
    public CustomerAppointmentKafkaDto transformToDto(final CustomerAppointment model) {
        return CustomerAppointmentKafkaDto.builder()
                ...
                .activities(
                        model.getActivities() != null
                                ? model.getActivities().stream()
                                        .map(customerAppointmentActivity ->
                                                CustomerAppointmentActivityKafkaDto.builder()
                                ...
                                : new ArrayList<>())
                .promotions(
                        model.getPromotions() != null
                                ? model.getPromotions().stream()
                                        .map(customerAppointmentPromotion ->
                                                CustomerAppointmentPromotionKafkaDto.builder()
                                                        .promotionId(customerAppointmentPromotion.getPromotionId())
                                                        .build())
                                        .toList()
                                : new ArrayList<>())
                .soldProducts(
                        model.getSoldProducts() != null
                                ? model.getSoldProducts().stream()
                                        .map(customerAppointmentSoldProduct ->
                                                CustomerAppointmentSoldProductKafkaDto.builder()
