...
public class TimeFilter implements IListFilter {
    ...
    public BooleanExpression buildGlobalSearchExpression(final Expression<?> expr, final String query) {
        ...
        return StringExpressions.lpad(castedExpr.hour().stringValue(), 2, '0').append(":")
    ...
    public BooleanExpression buildSearchExpression(final Expression<?> expr, final QueryPredicate query) {
        if (query == null || query.getSearchExpression() == null || query.getSearchExpression().isEmpty() || !(expr instanceof TimeExpression castedExpr)) {
            return null;
        }

        final String rawSearchExpr = query.getSearchExpression();
        final LocalTime searchExpression = LocalTime.parse(rawSearchExpr, DateTimeFormatter.ofPattern(
                "HH:mm[:ss]"
        ));
        ...
        switch (query.getSearchOperator()) {
            ...
            case STARTS_WITH -> result = StringExpressions.lpad(castedExpr.hour().stringValue(), 2, '0').append(":")
                    .append(StringExpressions.lpad(castedExpr.minute().stringValue(), 2, '0')).startsWithIgnoreCase(rawSearchExpr);
            case ENDS_WITH -> result = StringExpressions.lpad(castedExpr.hour().stringValue(), 2, '0').append(":")
