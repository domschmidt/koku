...
export class ListInlineListContainerComponent {
  ...
  contextMapping = input<{ [key: string]: KokuDto.AbstractListViewListContentContextDto }>();

  mappedContext = signal<{ [key: string]: any } | null | undefined>(undefined);
  ...
  constructor() {
    toObservable(this.contextMapping).pipe(takeUntilDestroyed(this.destroyRef)).subscribe((value) => {
      const resolvedContext: { [key: string]: any } = {};
      if (value) {
        ...
        for (const [currentKey, currentValue] of Object.entries(value || {})) {
          switch (currentValue['@type']) {
            case "endpoint": {
              ...
              observables.push(this.httpClient.request(castedValue.endpointMethod, endpointUrl).pipe(map((response) => {
