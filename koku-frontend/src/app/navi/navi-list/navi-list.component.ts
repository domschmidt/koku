...
export class NaviListComponent implements OnInit {
  ...
  private convertMainMenuRouterEntries(routes?: Route[], parentPath?: string) {
    ...
    for (const currentRoute of routes || []) {
      if ((!currentRoute.data || !currentRoute.data['hideInNav']) && currentRoute.path && currentRoute.path !== '**') {
        ...
        result.push({
          ...
          divider:
            currentRoute.data &&
            currentRoute.data['naviDivider'] &&
            (currentRoute.data['naviDivider'] === 'before' || currentRoute.data['naviDivider'] === 'after')
              ? currentRoute.data['naviDivider']
              : undefined,
