...
public class UserKafkaService {
    ...
    @Autowired
    public UserKafkaService(final KafkaTemplate<String, UserKafkaDto> userKafkaTemplate) {
        this.userKafkaTemplate = userKafkaTemplate;
    }

    public SendResult<String, UserKafkaDto> sendUser(final User user)
            throws ExecutionException, InterruptedException, TimeoutException {
        return this.userKafkaTemplate
                .send(UserKafkaDto.TOPIC, user.getId(), new UserToKafkaUserDtoTransformer().transformToDto(user))
