...
public class ProductToProductDtoTransformer {
    ...
    public ProductToProductDtoTransformer(
    ...
    public KokuProductDto transformToDto(final Product model) {
        ...
        return KokuProductDto.builder()
                ...
                .formattedPrice(!model.getPriceHistory().isEmpty() ? NumberFormat.getCurrencyInstance(Locale.GERMANY).format(model.getPriceHistory().getLast().getPrice()) : null)
    ...
    public Product transformToEntity(
            final Product model,
            final KokuProductDto updatedDto
        ...
        if (updatedDto.getManufacturerId() != null) {
            model.setManufacturer(this.entityManager.getReference(ProductManufacturer.class, updatedDto.getManufacturerId()));
        } else {
        ...
        if (updatedDto.getPrice() != null && (model.getPriceHistory().isEmpty() || !updatedDto.getPrice().equals(model.getPriceHistory().getLast().getPrice()))) {
            model.getPriceHistory().add(new ProductPriceHistoryEntry(
                    model,
                    updatedDto.getPrice()
            ));
        }
