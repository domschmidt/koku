...
public class ActivityToActivityDtoTransformer {
    ...
    public KokuActivityDto transformToDto(final Activity model) {
        return KokuActivityDto.builder()
                ...
                .name(model.getName())
                .approximatelyDuration(
                        model.getApproximatelyDuration() != null
                                ? LocalTime.MIN.plusNanos(
                                        model.getApproximatelyDuration().toNanos())
                                : null)
                .price(
    ...
    public Activity transformToEntity(final Activity model, final KokuActivityDto updatedDto) {

        if (updatedDto.getName() != null) {
            model.setName(updatedDto.getName());
        }
        if (updatedDto.getApproximatelyDuration() != null) {
            model.setApproximatelyDuration(Duration.ZERO.plusNanos(
                    updatedDto.getApproximatelyDuration().toNanoOfDay()));
        }
